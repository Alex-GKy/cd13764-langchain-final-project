...
class HealthRAGService:
    ...
    def __init__(self):
        ...
        self.relevance_threshold = 0.5  # Minimum similarity score for relevance
    ...
    def initialize(self, pdf_folder: str = "health_pdfs"):
        ...
        if not os.path.exists(pdf_folder):
            print(f"PDF folder '{pdf_folder}' not found. Creating sample PDFs...")
        ...
        print(f"Indexed {self.document_store.count_documents()} document chunks")
        
        # 4. Create RAG pipeline
        self.rag_pipeline = Pipeline()
        self.rag_pipeline.add_component("text_embedder", OpenAITextEmbedder())
        self.rag_pipeline.add_component("retriever", InMemoryEmbeddingRetriever(
    ...
    def search_documents(self, query: str) -> Tuple[bool, List[Document], List[float]]:
        """
        ...
        try:
            ...
            is_relevant = any(score >= self.relevance_threshold for score in scores) if scores else False
            
            if not is_relevant:
                print(f"No relevant documents found (max score: {max(scores) if scores else 'N/A'})")
